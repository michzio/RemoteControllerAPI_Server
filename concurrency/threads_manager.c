...
static void threads_manager_callback(callback_attr_t callback_attr, void *arg) {

    threads_manager_t *threads_manager = (threads_manager_t *) callback_attr;
    result_t *handler_result = (result_t *) arg;

    // 1. take mutex
    pthread_mutex_lock(&threads_manager->mutex);

    // 2. remove thread from threads manager
    if(threads_manager_remove_thread(threads_manager, pthread_self()) == FAILURE) {
...
result_t wait_for_thread(threads_manager_t *threads_manager, runner_t runner, runner_attr_t runner_attr) {
    ...
    // 3. create new thread
    if( (thread = thread_create(runner, runner_attr)) == NULL) {
...
result_t wait_for_connection_thread(threads_manager_t *threads_manager, connection_handler_t conn_handler, sock_fd_t conn_sock_fd) {
    ...
    conn_thread_runner_attr_fill(conn_thread_runner_attr, conn_handler, conn_sock_fd, threads_manager_callback, (callback_attr_t) threads_manager);
...
result_t wait_for_datagram_thread(threads_manager_t *threads_manager, datagram_handler_t datagram_handler, sock_fd_t sock_fd, const struct sockaddr *peer_addr, const char *datagram) {
    ...
    datagram_thread_runner_attr_fill(datagram_thread_runner_attr, datagram_handler, sock_fd, peer_addr, datagram, threads_manager_callback, (callback_attr_t) threads_manager);
